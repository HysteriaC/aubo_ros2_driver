...
class TcpClientService(Node):
    def __init__(self):
        ...
        self.tcp_ip = (
            self.get_parameter("tcp_client.ip").get_parameter_value().string_value
        )
        self.tcp_port = (
            self.get_parameter("tcp_client.port").get_parameter_value().integer_value
        )
        self.robot_prefix = (
            self.get_parameter("tcp_client.robot_prefix")
            .get_parameter_value()
            .string_value
        )
    ...
    def handle_service(self, request, response):
        ...
        try:
            ...
            self.get_logger().warn(
                f"[JSON] Failed to parse params, using [] instead. err={e}"
            )
            params = []
